  ...
  head: () => ({
    meta: [
      { title: "Skenis.lt | Programuojami Google atsiliepimų QR stendai" },
      {
        name: "description",
        content:
          "Programuojamos akrilinės kortelės ir stendai, kurie nukreipia klientus tiesiai į Google atsiliepimų puslapį."
      },
      { property: "og:title", content: "Skenis.lt" },
      {
        property: "og:description",
        content:
          "Daugiau Google atsiliepimų su išmaniais programuojamais QR stendais."
      }
    ]
  }),
  component: HomePage
      ...
      <header className="absolute left-0 right-0 top-0 z-20">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 text-white sm:px-8">
          ...
          <div className="hidden items-center gap-7 text-sm font-medium md:flex">
            ...
            <Link to="/admin" className="transition hover:text-brand-100">
              Admin
            </Link>
          </div>
        </nav>
        ...
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {publicProducts.map((product) => (
              <article key={product.name} className="rounded-lg border border-line bg-white p-6 shadow-panel">
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-5 sm:px-8 lg:grid-cols-[0.85fr_1.15fr]">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {benefits.map((benefit) => (
              <div key={benefit} className="rounded-lg border border-white/10 bg-white/[0.06] p-5 text-sm leading-6 text-slate-100">
                {benefit}
              </div>
            ))}
          </div>
      ...
      <footer className="border-t border-line bg-white">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-8 text-sm text-slate-600 sm:px-8 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-5">
            <Link to="/privatumo-politika" className="hover:text-brand-700">
              Privatumo politika
            </Link>
            <Link to="/taisykles" className="hover:text-brand-700">
              Taisyklės
            </Link>
